...
const quizChecks = async (req, res, next) => {
    ...
    const allowedPathsNewUser = ['/', '/lobby-new', '/lobby-join', '/cookie-policy', '/tandc', '/api/logs'];
    ...
    if (res.locals.userData && !allowedPathsCurrentUser.includes(req.path) && !Object.keys(req.query).some(key => allowedQueries.includes(key))) { // Makes sure there is userData
        ...
        if (req.path !== userData.progress) {
            req.flash('error', 'You ended up at the wrong place, but you are back now!');
